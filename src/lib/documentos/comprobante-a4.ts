/**
 * Comprobante A4 de venta: CSS, carga de datos y armado de la "hoja".
 *
 * Vive aca para que lo puedan usar tanto la impresion de UNA venta como la
...
/** CSS compartido por el comprobante (una hoja o varias). */
export const COMPROBANTE_A4_CSS = `  * { box-sizing: border-box; }
...
/**
 * Arma el bloque <div class="hoja"> de una venta.
 * Devuelve null si la venta no existe para esa empresa.
 */
export async function hojaComprobanteA4(
  sb: AppSupabaseClient,
  empresaId: string,
  ventaId: string
): Promise<{ hoja: string; numeroControl: string } | null> {
  ...
  if (!venta) return null;
    ...
    // Podria leerse de ferretodo.empresas si se completa el campo 'ciudad'.
